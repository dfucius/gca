> forked from https://github.com/mrgoonie/cmai

# `gca` - AI Commit Message Generator
...
- Install the script globally as `gca`
...
- Rename `git-commit.sh` to `gca.sh`
...
gca --use-custom http://your-api-endpoint
```
...
```bash
gca --model qwen/qwen-2.5-coder-32b-instruct
```
...
gca --model "your-model-name"

# Debug mode with LMStudio
gca --debug --use-lmstudio
```
...
gca --use-custom http://my-api.com

# Use custom provider with specific model
gca --use-custom http://my-api.com --model my-custom-model
```

# Use a different model

gca --use-ollama --model qwen-coder:7b

# Use Ollama with debug and push

gca --use-ollama --debug --push
...
gca --base-url https://api.example.com/v1

# Combine multiple flags
gca --debug --push --model your-model --base-url https://api.example.com/v1
````
...
└── usr/
  └── local/
    └── bin/
      └── gca -> ~/git-commit-ai/git-commit.sh
```
...
1. **No API key found**
   ...
   - Run `gca your_openrouter_api_key` to configure
...
3. Commit your changes (using `gca` 😉)
